...
let ctx = null;
let isDrawing = false;
let isDragging = false; // Для панорамування (інструмент "Рука")
let currentTool = 'brush'; // 'brush', 'text', 'eraser' або 'hand'
let lastX = 0;
let lastY = 0;
let bgImage = new Image();
let canvasOffsetX = 0; // Зсув для панорамування
let canvasOffsetY = 0;
let dragStartX = 0;
let dragStartY = 0;
...
function initHomeworkCanvas(imageUrl) {
  ...
  // Встановлюємо правильний клас курсора на wrapper
  updateCanvasCursor();

  // Тач-події для мобільних — окрема обробка для кожного інструменту
...
// =========================================================================
// Інструмент "Рука" — панорамування зображення
// =========================================================================
function updateCanvasCursor() {
  ...
  wrapper.classList.remove('dragging', 'drawing-mode');
...
// Обробники тач-подій для canvas
function handleTouchStart(e) {
  if (e.touches.length !== 1) return;
  
  const touch = e.touches[0];
  
  // Якщо вибрано "Рука" — починаємо панорамування
  if (currentTool === 'hand') {
    e.preventDefault();
    isDragging = true;
    dragStartX = touch.clientX - canvasOffsetX;
    dragStartY = touch.clientY - canvasOffsetY;
    const wrapper = document.getElementById('canvas-wrapper');
    if (wrapper) wrapper.classList.add('dragging');
    return;
  }
  
  // Для тексту — не блокуємо, даємо клавіатурі з'явитися
...
function handleTouchMove(e) {
  ...
  // Панорамування для "Рука"
  if (currentTool === 'hand' && isDragging) {
    e.preventDefault();
    const wrapper = document.getElementById('canvas-wrapper');
    
    // Рухаємо wrapper через CSS transform
    canvasOffsetX = touch.clientX - dragStartX;
    canvasOffsetY = touch.clientY - dragStartY;
...
function setupToolbarListeners() {
  ...
  function setActiveTool(activeBtn) {
    const allTools = [brushBtn, textBtn, eraserBtn, handBtn].filter(Boolean);
    allTools.forEach(b => b.classList.remove('tool-active'));
    activeBtn.classList.add('tool-active');
    updateCanvasCursor();
  }

  if (brushBtn && textBtn && eraserBtn && handBtn) {
    ...
    handBtn.onclick = () => { if (activeTextarea) finalizeLiveText(); currentTool = 'hand'; setActiveTool(handBtn); };
  }

  // Додаємо панорамування мишею для інструменту "Рука" на wrapper
  const wrapper = document.getElementById('canvas-wrapper');
  if (wrapper) {
    ...
    document.addEventListener('mousemove', (e) => {
      if (currentTool !== 'hand' || !isDragging) return;
