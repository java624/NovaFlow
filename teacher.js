...
document.addEventListener("DOMContentLoaded", function() {
  verifyTeacherSession().then(() => {
    autoProcessPassedLessons().then(() => {
...
// =========================================================================
// АВТОМАТИЧНЕ СПИСАННЯ УРОКІВ, ЯКІ МИНУЛИ
// =========================================================================
async function autoProcessPassedLessons() {
  try {
    const now = new Date().toISOString();
    const { data: passedLessons, error } = await teacherSupabase
      .from('lessons')
      .select('id, student_id, end_time')
      .lt('end_time', now);

    if (error || !passedLessons || passedLessons.length === 0) return;

    // Групуємо уроки по студентах
    const studentDeductions = {};
    const lessonIdsToDelete = [];

    passedLessons.forEach(lesson => {
      studentDeductions[lesson.student_id] = (studentDeductions[lesson.student_id] || 0) + 1;
      lessonIdsToDelete.push(lesson.id);
    });

    // Оновлюємо баланси студентів
    for (const studentId of Object.keys(studentDeductions)) {
      const deduction = studentDeductions[studentId];
      ...
      if (profile) {
        let newBalance = (profile.lessons_left || 0) - deduction;
    ...
    // Видаляємо проведені уроки з таблиці
    ...
    console.log(`Автоматично списано ${lessonIdsToDelete.length} минулих уроків.`);
  } catch (err) {
    console.error("Помилка при автоматичному списанні уроків:", err);
  }
}
