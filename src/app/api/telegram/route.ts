...
// Глобальна змінна для синглтона бота
let botInstance: Bot | null = null;

// Функція лінивої ініціалізації бота (викликається тільки під час реального HTTP-запиту)
function getBot(): Bot {
  if (botInstance) {
    return botInstance;
  }
  ...
  bot.command("start", async (ctx) => {
    ...
    // Звичайне вітальне повідомлення
    await ctx.reply(
      "👋 *Вітаємо в NovaFlow!*\n\n" +
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Віддаємо 200 OK Telegram-у, щоб він не повторював запит нескінченно при помилках
