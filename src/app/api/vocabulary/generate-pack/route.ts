...
export async function POST(req: Request) {
  try {
    ...
    for (let i = 0; i < words.length; i++) {
      const item = await fetchLiveDictionaryAndTranslation(words[i], level as CEFRLevel, i);
...
[
  {
    ...
    "definition": "Просте пояснення значення цільовою мовою",
...
- Для кожного слова згенеруй точний переклад, IPA-транскрипцію, дефініцію, 1-2 контекстні речення з перекладом.
- Якщо слово є фразою (наприклад "затримка рейсу"), збережи його як фразу.
`;
  ...
  return parsed.map((p: any, idx: number) => ({
    ...
    primaryTranslation: p.primaryTranslation || 'Переклад',
...
async function fetchLiveDictionaryAndTranslation(
  ...
  // A. Fetch Translation from MyMemory
  try {
