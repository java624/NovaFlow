...
Твоє завдання — перетворити список вхідних слів/фраз у масив багатих навчальних карток словника.
...
КРИТИЧНО ВАЖЛИВО — НАПРЯМОК ПЕРЕКЛАДУ ТА МОВА:
- Автоматично визнач мову кожного вхідного слова/фрази.
- Поле "word" має містити слово ЦІЛЬОВОЮ МОВОЮ (${targetLanguage}). 
  Якщо вхідне слово вказано рідною мовою (наприклад, українською "літак") і цільова мова "${targetLanguage}" (наприклад, German), то "word" буде "Flugzeug".
  Якщо вхідне слово вже вказано цільовою мовою (наприклад, "Flugzeug"), то збережи його як "word": "Flugzeug".
- Поле "primaryTranslation" має містити переклад українською мовою. 
  Якщо цільова мова сама є "Ukrainian", то поле "primaryTranslation" має містити переклад англійською або німецькою мовою відповідно.
- Поле "definition" має бути простим поясненням значення слова ЦІЛЬОВОЮ МОВОЮ (${targetLanguage}). Якщо цільова мова Ukrainian, пояснення може бути українською або англійською.
- Поле "contextExamples" має містити 1-2 приклади речень ЦІЛЬОВОЮ МОВОЮ (${targetLanguage}) разом з перекладом на українську мову (або навпаки, якщо цільова мова Ukrainian).
- Поле "mnemonicHint" має бути яскравою підказкою-асоціацією українською мовою.
...
[
  {
    ...
    "primaryTranslation": "переклад мовою перекладу",
    "alternativeTranslations": ["альтернативний переклад 1", "альтернативний переклад 2"],
    "definition": "пояснення значення цільовою мовою",
    "mnemonicHint": "підказка-асоціація українською",
    "collocations": ["словосполучення 1 цільовою мовою", "словосполучення 2 цільовою мовою"],
    "contextExamples": [
      { "sentence": "Приклад 1 цільовою мовою", "translation": "Переклад 1 українською" },
      { "sentence": "Приклад 2 цільовою мовою", "translation": "Переклад 2 українською" }
    ],
...
- Збережи оригінальний порядок слів.
- Згенеруй правильну частину мови (partOfSpeech) та рівень (cefrLevel).
`;
