...
export default function LanguageHeader() {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <a href="/" className="flex items-center gap-2 shrink-0">
              <Image src="/img/logo.svg" alt="NovaFlow Logo" width={36} height={36} className="w-8 h-8 lg:w-9 lg:h-9" />
              <div className="flex flex-col leading-tight">
                <span className="text-lg lg:text-xl font-bold text-gray-900 -mb-0.5">NovaFlow</span>
                <span className="text-[10px] lg:text-[11px] font-semibold tracking-[0.15em] text-purple-600">LANGUAGE SCHOOL</span>
              </div>
            </a>
            ...
            >
              <span className={`block w-5 h-0.5 bg-white rounded transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`} />
              <span className={`block w-5 h-0.5 bg-white rounded transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''}`} />
              <span className={`block w-5 h-0.5 bg-white rounded transition-all duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
            </button>
      ...
      {/* Mobile & Tablet Menu — outside header to avoid stacking context issues */}
      <div className={`fixed top-0 right-0 h-full w-80 bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out lg:hidden ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex flex-col pt-20 pb-8 px-6 h-full">
          <button onClick={() => setIsMenuOpen(false)}
            className="absolute top-4 right-4 w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors" aria-label="Close Menu">
