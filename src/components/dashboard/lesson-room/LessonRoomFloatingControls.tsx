...
export default function LessonRoomFloatingControls({
  ...
  return (
    <footer className="fixed bottom-0 sm:bottom-6 left-0 sm:left-1/2 sm:-translate-x-1/2 z-40 flex items-center justify-center gap-1 sm:gap-3 px-2 sm:px-5 py-2 sm:py-3 w-full sm:w-auto rounded-none sm:rounded-full backdrop-blur-2xl bg-slate-900/80 sm:bg-zinc-900/85 border-t sm:border border-white/10 sm:border-white/15 shadow-2xl shadow-black/80 pb-[env(safe-area-inset-bottom)]">
      {/* Mic toggle */}
      <ControlButton
        active={!micMuted}
        onClick={onMicToggle}
      ...
      <ControlButton
        ...
        inactiveClass="bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-white"
        icon={<span className="text-base sm:text-base">✋</span>}
      />
      ...
      <button
        onClick={onLeave}
        className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-rose-600 hover:bg-rose-500 text-white flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95 shadow-lg shadow-rose-600/30"
        title="Вийти з уроку"
      >
...
function ControlButton({ active, onClick, icon, tooltip, activeClass, inactiveClass }: ControlButtonProps) {
  return (
    <button
      onClick={onClick}
      title={tooltip}
      className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95 shadow-md ${
