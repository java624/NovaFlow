...
interface StudentProfileModalProps {
  ...
  onAssignVocabulary?: (student: StudentProfile) => void;
}
      ...
      <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-2xl w-full max-w-lg relative">
        ...
        <form onSubmit={saveProfile} className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="submit" className="flex-1 min-w-[120px] inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-purple-600 to-purple-500 rounded-xl shadow-md hover:shadow-lg">💾 Зберегти</button>
            {onAssignVocabulary && (
              <button
                type="button"
                onClick={() => {
                  onClose();
                  onAssignVocabulary(student);
                }}
                className="px-4 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-750 rounded-xl transition-colors"
                title="Задати пакет слів через ШІ"
              >
                📦 Задати пакет слів
              </button>
            )}
