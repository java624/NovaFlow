...
export default function CreateMaterialModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white rounded-3xl max-w-xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-100 p-6 space-y-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs sm:text-sm">
          {/* Title */}
          <div>
            <label className="block font-semibold text-gray-700 mb-1">Назва матеріалу *</label>
            <input
              type="text"
              required
              ...
              className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500"
            />
          </div>

          {/* Description */}
          <div>
            <label className="block font-semibold text-gray-700 mb-1">Опис або інструкція (необов'язково)</label>
            <textarea
              rows={3}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block font-semibold text-gray-700 mb-1">Категорія</label>
              <select
                value={category}
                onChange={(e) => setCategory(e.target.value as MaterialCategory)}
                className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500/20"
              >
            ...
            <div>
              <label className="block font-semibold text-gray-700 mb-1">Кому призначити</label>
              <select
                value={studentId}
          ...
          <div>
            <label className="block font-semibold text-gray-700 mb-1">Тип вмісту</label>
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                🔗 Зовнішнє посилання (YouTube, Web)
              </button>
            </div>
          </div>
          ...
          {attachmentType === 'file' ? (
            <div>
              <label className="block font-semibold text-gray-700 mb-1">Виберіть файл</label>
              <input
                type="file"
            ...
            <div>
              <label className="block font-semibold text-gray-700 mb-1">URL Посилання</label>
              <input
                type="url"
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl text-xs shadow-md shadow-purple-500/20 disabled:opacity-50"
            >
